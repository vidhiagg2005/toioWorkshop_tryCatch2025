/*
 * ToioTeachingSandbox_WhileLoopOnly_Sim.cs
 *
 * Simulator version (no BLE).
 * Focus: while loops with simulated toio cubes.
 */
...
public class ToioTeachingSandbox_WhileLoopOnly_Sim : MonoBehaviour
{
    ...
    async void Start()
    {
        ...
        Debug.Log("[WhileLoopSandbox:SIM] Looking for simulated cubes...");
        ...
        if (cubes.Length == 0)
        {
            Debug.LogError("[WhileLoopSandbox:SIM] No simulated cubes found. " +
                           "Ensure 'Simulator/Playground' with cubes is in the scene.");
            enabled = false;
            return;
        }

        Debug.Log($"[WhileLoopSandbox:SIM] Connected {cubes.Length} simulated cube(s).");
        ...
        Debug.Log("[WhileLoopSandbox:SIM] Ready to teach WHILE loops!");
    }
    ...
    {
        Debug.Log("[WhileLoopSandbox:SIM] Running student WHILE loop pattern...");

        // --- START: Students edit below this line ---
        /*
         * 🚀 Task: Use a WHILE loop to repeat actions while a condition is TRUE.
         *
         * Example pattern below spins 3 times, beeping each spin.
         *
         * Helpers you can use:
        ...
        Debug.Log("[WhileLoopSandbox:SIM] Student pattern finished!");
    }
    ...
    {
        // Simulator may not have audio; fall back to quick spin feedback.
        bool played = false;
        try
        {
            c.PlayPresetSound(3);
